...
class UrbanExposure(BaseModel):
    segment_id: str = Field(min_length=1)
    measured_at: datetime
    shadow_index: float = Field(ge=0, le=1)
    sunlight_minutes_last_hour: float = Field(ge=0, le=60)
    geothermal_index: float = Field(ge=0, le=1)
    heat_island_index: float = Field(ge=0, le=1)
...
class MobilitySnapshot(BaseModel):
    ...
    exposure_score: float = Field(ge=0)
...
class SmartDeicingMapModel(BaseModel):
    ...
    urban_exposures: list[UrbanExposure] = Field(default_factory=list)
    mobility_snapshots: list[MobilitySnapshot] = Field(default_factory=list)
